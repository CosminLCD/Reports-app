...
import type { ManualFields, BrandColor, CustomField } from '@/types'
...
export function Step1ManualFields({ fields, onChange, customFields, customValues, onCustomChange, onNext, airtableOptions, fieldNames, preFilled, onDismissPreFilled }: Step1Props) {
  ...
  const defaultBrandColors: BrandColor[] = [
    { label: 'Culoare primară', hex: '' },
    { label: 'Culoare secundară', hex: '' },
  ]

  const brandColors = fields.brandColors ?? defaultBrandColors

  const updateBrandColor = (index: number, key: keyof BrandColor, value: string) => {
    const updated = brandColors.map((c, i) => i === index ? { ...c, [key]: value } : c)
    onChange({ ...fields, brandColors: updated })
  }
    ...
    <div className="space-y-5">
      ...
      <div className="border-t pt-4">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">
          Culori brand client <span className="font-normal normal-case text-gray-400">(opțional — folosite de AI pentru soluții de contrast)</span>
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {brandColors.map((color, i) => (
            <Field key={i} label={color.label}>
              <div className="flex items-center gap-2">
                <input
                  type="color"
                  value={isValidHex(color.hex) ? color.hex : '#000000'}
                  onChange={(e) => updateBrandColor(i, 'hex', e.target.value)}
                  className="h-9 w-10 cursor-pointer rounded border border-gray-300 bg-white p-0.5"
                  aria-label={`Color picker pentru ${color.label}`}
                />
                <input
                  type="text"
                  value={color.hex}
                  ...
                  aria-label={`Valoare hex pentru ${color.label}`}
                />
              </div>
            </Field>
          ))}
